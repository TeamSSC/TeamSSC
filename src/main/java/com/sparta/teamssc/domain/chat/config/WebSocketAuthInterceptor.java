...
public class WebSocketAuthInterceptor implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        // 메시지 전송 이거나 구독 요청
                ...
                // WebSocket 메시지 전송 및 구독 시 세션에 저장된 토큰을 사용하여 인증 정보 설정
                SecurityContext securityContext = (SecurityContext) accessor.getSessionAttributes().get("SPRING_SECURITY_CONTEXT");
                if (securityContext != null) {
                    SecurityContextHolder.setContext(securityContext);
                    log.info("메시지 처리 시 SecurityContext 재사용: {}", securityContext.getAuthentication());
                } else {
                    log.warn("메시지 처리 실패: 세션에 SecurityContext가 없음");
                }
            }
